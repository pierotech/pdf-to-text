...
app.post("/upload", async (c) => {
  try {
    ...
    await c.env.BUCKET.put(csvKey, new TextEncoder().encode(finalCsvOutput), {
      ...
      customMetadata: { persist: "true" }, // 🚀 Ensures file stays in R2
    });

    // 9) Debug: Check if file exists in R2
    const storedCsv = await c.env.BUCKET.get(csvKey);
    if (!storedCsv) {
      console.error("CSV file was not stored successfully:", csvKey);
